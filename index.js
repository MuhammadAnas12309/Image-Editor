...
filterOptions.forEach((option) => {
  option.addEventListener("click", () => {
    //adding click event listener to all filter bonus
...
const updateFilter = () => {
  ...
  const selectedFilter = document.querySelector(".filter .active"); //geeting slected filter btn
...
rotateOptions.forEach((option) => {
  option.addEventListener("click", () => {
    ...
    if (option.id === "left") {
      ...
      rotate += 90; //clicked btn is left rotate, increment rotate value by +90
...
const resetFilter = () => {
 // resetting all variables value to its default value
  ...
  filterOptions[0].click(); //checking brightness btn, so the brightnedd selected by default
  applyFilters();
};
...
const saveImage = () => {
 ...
 //applying user selected filters to canvas filter
  ...
  link.href = canvas.toDataURL(); // passing <a> tag here value to canvas data url
  link.click(); // clicking <a> tag the image download
}
